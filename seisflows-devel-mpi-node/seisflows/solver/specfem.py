...
from seisflows.tools.specfem import getpar, setpar, check_source_names, check_station_names
...
class Specfem:
    ...
    ***
    ...
    def __init__(self, data_format="ascii",  materials="acoustic",
                 density=False, nproc=1, ntask=1, ngpu=2, njobs=2, multi_nodes=False, parallel_backend="loky", attenuation=False,
        ...
        self.station_prefix = "STATIONS"
        self.ngpu = ngpu
        self.njobs = njobs
        self.parallel_backend = parallel_backend
        # Define internally used directory structure
        self.path = Dict(
            solver=os.path.join(workdir, 'scratch', 'solver'),
    ...
    def check(self):
        ...
        assert(glob(os.path.join(self.path.specfem_data,
                                 f"{self.station_prefix}*"))), (
            f"No station files with prefix {self.station_prefix} found in DATA/")
        ...
        # Check that the number of tasks/events matches the number of events
        self._station_names = check_station_names(
            path_specfem_data=self.path.specfem_data,
            station_prefix=self.station_prefix, ntask=self.ntask
        )
    ...
    def station_names(self):
        ...
        if self._station_names is None:
            self._station_names = check_station_names(
                path_specfem_data=self.path.specfem_data,
                source_prefix=self.station_prefix, ntask=self.ntask
            )
    ...
    def station_name(self):
        ...
        """
        return self._station_names[get_task_id()]
    ...
    def setup(self):
        ...
        self._initialize_mainsolver_directories()        # 2022-11-14
        self._export_starting_models()

    def forward_simulation(self, executables=None, save_traces=False,
                           export_traces=False, no_backward=False, **kwargs):
        """
        ...
        iters = ceil(self.ntask/(self.nproc)) # The number of iters of all GPU.

        logger.debug(f"Updating DATA/Par_file of each xxxx folder no_backward={no_backward}")
        # Check and change DATA/Par_file
        for idx in range(self.ntask):
            DATA_FILE = os.path.join(self.path.solver, "%04d"%(idx+1), "DATA/Par_file")
            #logger.debug(f"Updating {DATA_FILE}")
            setpar(key="SIMULATION_TYPE", val="1", file=DATA_FILE)
            setpar(key="SAVE_FORWARD", val=".true.", file=DATA_FILE)
            setpar(key="SAVE_MODEL", val=".false.", file=DATA_FILE)

            # When perform line searching, set NO_BACKWARD_RECONSTRUCTION=True for saving IO time.
            if no_backward:
                setpar(key="NO_BACKWARD_RECONSTRUCTION", val=".true.", file=DATA_FILE)
                setpar(key="NTSTEP_BETWEEN_COMPUTE_KERNELS", val="999999", file=DATA_FILE)
            else:
                setpar(key="NO_BACKWARD_RECONSTRUCTION", val=".flase.", file=DATA_FILE)
                setpar(key="NTSTEP_BETWEEN_COMPUTE_KERNELS", val="1", file=DATA_FILE)

        logger.debug(f"Updating DATA/Par_file of each mainsolver_xxxx folder")
        # Check and change mainsolver_xxxx/DATA/Par_file for mpi running
        for _iter in range(iters):
            main_solver_path = os.path.join(self.path.solver, "mainsolver_iter%04d"%(_iter))
            DATA_FILE = os.path.join(main_solver_path, "DATA/Par_file")
            runs = len(glob(os.path.join(main_solver_path, "run*")))
            setpar(key="SIMULATION_TYPE", val="1", file=DATA_FILE)
            setpar(key="SAVE_FORWARD", val=".true.", file=DATA_FILE)
            setpar(key="SAVE_MODEL", val=".false.", file=DATA_FILE)
            setpar(key="NUMBER_OF_SIMULTANEOUS_RUNS", val=f"{self.nproc}", file=DATA_FILE)
        ...
        def work_around_gpu(group_idx, no_backward=no_backward):
            # Get the id of GPU
            gpuid = group_idx%self.ngpu
            gpuid = gpuid-2 if gpuid>1 else gpuid
            # Get the path of mainsolver
            #  mainsolver_path = os.path.join (self.path.solver, f"mainsolver_dev{gpuid}")
            mainsolver_path = os.path.join (self.path.solver, "mainsolver_iter%04d"%(group_idx))
            # Change to mainsovler of the current gpu
            unix.cd(mainsolver_path)

            # Calling subprocess.run() for each of the binary executables listed
            for exc in executables:
                # e.g., fwd_mesher.log
                stdout = os.path.join(mainsolver_path, f"fwd_{self._exc2log(exc)}.log")
                self._run_binary(executable=exc, stdout=stdout, gpuid=gpuid, cwd=mainsolver_path, no_backward=no_backward)

        # Parallel for GPU
        logger.debug(f"Total {self.nproc}*{self.ngpu} jobs on {self.ngpu} gpu.")
        Parallel(n_jobs=self.njobs, backend=self.parallel_backend)(delayed( work_around_gpu )(idx) for idx in range(iters))

        # Work around SPECFEM's version dependent file names
        unix.cd(self.path.solver)
        # Warp function for parallel process
        def work_around(idx):
            dir_name = "%04d"%(idx+1)
            logger.debug(f"cd to {dir_name} for modifing the output .su file")
            # change directory to runxxxx
            unix.cd(os.path.join(self.path.solver, dir_name))
            if self.data_format.upper() == "SU":
                for tag in ["d", "v", "a", "p"]:
            ...
            unix.mv(
                src=glob(os.path.join("OUTPUT_FILES", self.data_wildcard())),
                dst=os.path.join(self.path.solver, dir_name, "traces", "syn")
            )

            # !!!!!!!IMPORTANT!!!!!!!!!!
            # !!cd back to mainsolver!!!
            # !!!!!!!IMPORTANT!!!!!!!!!!
            unix.cd(self.path.solver)

        # Parallel process the files
        Parallel(n_jobs=self.nproc, backend=self.parallel_backend)(delayed( work_around )(idx) for idx in range(self.ntask))
    ...
    def adjoint_simulation(self, executables=None, save_kernels=False,
        ...
        run_dirs = sorted(glob('*'))
        run_dirs = [_dir_ for _dir_ in run_dirs if 'mainsolver' not in _dir_]
        
        for _dir_ in run_dirs:
            # change directory to runxxx
            unix.cd(os.path.join(self.path.scratch, _dir_))
            # modify the Par_file for runxxxx/DATA/Par_file
            setpar(key="SIMULATION_TYPE", val="3", file="DATA/Par_file")
            setpar(key="SAVE_FORWARD", val=".false.", file="DATA/Par_file")
            setpar(key="SAVE_MODEL", val=".false.", file="DATA/Par_file")

            unix.rm("SEM")
            unix.ln("traces/adj", "SEM")
            # change director back to scratch
            unix.cd(self.path.scratch)

        logger.debug(f"Updating DATA/Par_file of each mainsolver_xxxx folder for adjoint simulation")
        iters = ceil(self.ntask/(self.nproc))
        # Check and change mainsolver_xxxx/DATA/Par_file for mpi running
        for _iter in range(iters):
            main_solver_path = os.path.join(self.path.solver, "mainsolver_iter%04d"%(_iter))
            DATA_FILE = os.path.join(main_solver_path, "DATA/Par_file")
            runs = len(glob(os.path.join(main_solver_path, "run*")))
            setpar(key="SIMULATION_TYPE", val="3", file=DATA_FILE)
            setpar(key="SAVE_FORWARD", val=".false.", file=DATA_FILE)
            setpar(key="SAVE_MODEL", val=".false.", file=DATA_FILE)
            setpar(key="NUMBER_OF_SIMULTANEOUS_RUNS", val=f"{self.nproc}", file=DATA_FILE)

        unix.cd(self.path.solver)
        source_names = ["%04d"%(idx+1) for idx in range(self.ntask)]

        def work_around_gpu_adj(group_idx):
            # Get the id of GPU
            gpuid = group_idx%self.ngpu
            # Get the path of mainsolver
            #mainsolver_path = os.path.join (self.path.solver, f"mainsolver_dev{gpuid}")
            ...
            for exc in executables:
                ...
                self._run_binary(executable=exc, stdout=stdout, gpuid=gpuid, no_backward=False)

        logger.info("Running adjoint simulation")
        # Parallel for GPU
        Parallel(n_jobs=self.njobs, backend=self.parallel_backend)(delayed( work_around_gpu_adj )(idx) for idx in range(iters))

        # Rename kernels to work w/ conflicting name conventions
        # Change directory so that the rename doesn't affect the full path

        unix.cd(self.path.solver)
        def work_around_adj(idx, export_kernels=export_kernels, save_kernels=save_kernels):
            # change directory to runxxx
            source_name = run_dirs[idx]
            unix.cd( os.path.join(self.path.solver, source_name) )
            # change directory to runxxxx/OUTPUT_FILES
            unix.cd(self.kernel_databases)
            logger.debug(f"renaming output event kernels: 'alpha' -> 'vp'")
            for tag in ["alpha", "alpha[hv]", "reg1_alpha", "reg1_alpha[hv]", "c_acoustic", "rho_acoustic"]:
            ...
            if export_kernels:
                export_kernels = os.path.join(self.path.eval_grad, 'kernels', source_name)
                unix.mkdir(export_kernels)
                logger.debug(f"export_kernels: {export_kernels}")
                for par in self._parameters:
                    unix.cp(src=glob(f"*{par}_kernel{self._ext}"),
                            dst=export_kernels)

            if save_kernels:
                save_kernels = os.path.join(self.path.eval_grad, 'kernels', source_name)
                unix.mkdir(save_kernels)
                logger.debug(f"save_kernels: {save_kernels}")
                for par in self._parameters:
                    unix.mv(src=glob(f"*{par}_kernel{self._ext}"), dst=save_kernels)
            #   Back to solver
            unix.cd(self.path.solver)

        logger.info("Modifying kernels of gradients")
        # Parallel process the files
        Parallel(n_jobs=self.nproc, backend=self.parallel_backend)(delayed( work_around_adj )(idx) for idx in range(len(run_dirs)))
    ...
    def _run_binary(self, executable, stdout="solver.log", with_mpi=True, gpuid=0, host=None, cwd=None, no_backward=False):
        """
        ...
        if self._mpiexec and with_mpi:
            if "xmesh" in executable:
                executable = f"{self._mpiexec} -n {self.nproc} {executable}"
            else:
                if self.multi_nodes and no_backward: #no_backward=True: do not save boundary to disk
                    executable = f"export CUDA_VISIBLE_DEVICES={gpuid}&&{self._mpiexec} -n {self.nproc} -f SERVICES {executable}"
                else:
                    executable = f"export CUDA_VISIBLE_DEVICES={gpuid}&&{self._mpiexec} -n {self.nproc} {executable}"
                #executable = f"export CUDA_VISIBLE_DEVICES={gpuid}&&{self._mpiexec} -n {self.nproc} {executable}"
                #else:
                #    executable = f"export CUDA_VISIBLE_DEVICES={gpuid}&&{self._mpiexec} -n {self.nproc} {executable}"
            #executable = f"export CUDA_VISIBLE_DEVICES={gpuid}&&{self._mpiexec} -n {self.nproc} {executable}"
        ...
        while retry:
            try:
                with open(stdout, "w") as f:
                    status = subprocess.run(executable, shell=True, check=True, stdout=f,
                                            stderr=f)
                retry = False

            except (subprocess.CalledProcessError, OSError) as e:
                logger.critical(
                    msg.cli(f"current direcotry: {actual_cwd} "
                            f"excepted directory: {excepted_cwd} "
                            "The external numerical solver has returned a "
                            "nonzero exit code (failure). Consider stopping any "
                            "currently running jobs to avoid wasted "
                            "computational resources. Check '"
                            f"{stdout}' for the solvers stdout log "
                            "message. The failing command and error message are:",
                            items=[f"exc: {executable}", f"err: {e}"],
                            header="external solver error",
                            border="=")
                )
                logger.debug(f"retry running executable with cmd: '{executable}'")
                if e.returncode != 0:
                    retry = True
    ...
    def import_model(self, path_model):
        ...
        #mainsolver_path = [os.path.join (self.path.solver, f"mainsolver_dev{gpuid}/DATA") for gpuid in range(self.ngpu)]
        iters = ceil(self.ntask/(self.nproc)) # The number of iters of all GPU.
        mainsolver_path = [os.path.join (self.path.solver, "mainsolver_iter%04d/DATA"%group_idx) for group_idx in range(iters)]
        #mainsolver_path.append( os.path.join(self.path.solver, "mainsolver/DATA") )
        for dst in mainsolver_path:
            for _file in src:
                dst_file = os.path.join(dst, os.path.basename(_file))
                # If the target file is an symlink or an invalid symlink, unlink it.
                if Path(dst_file).is_symlink():
                    os.unlink(dst_file)
                # If the target file is an exact file, delete it from disk.
                if os.path.exists(dst_file):
                    unix.rm(dst_file)
                unix.ln(_file, dst)

        # for solver/mainsolver, we copy the files to the folder.
        mainsolver_path_main = os.path.join(self.path.solver, "mainsolver/DATA")
        for _file in src:
            dst_file = os.path.join(mainsolver_path_main, os.path.basename(_file))
            if os.path.exists(dst_file):
                unix.rm(dst_file)
            unix.cp(_file, mainsolver_path_main)

    def _initialize_mainsolver_directories(self,):
        """
        2022-11-14
        Make a solver dir for mpi modeling
        """
        # MKDIR for mainsolver

        logger.info(f"initializing main solver directory")

        # Starting from a fresh working directory
        if not os.path.exists(self.path.mainsolver):
            unix.mkdir(self.path.mainsolver)
        else:
            #unix.rm(self.path.mainsolver)
            unix.mkdir(self.path.mainsolver)

        # Define a constant list of required !!!MPIRUN!! SPECFEM dir structure, relative cwd
        _required_structure = ["bin", "DATA", self.model_databases, self.kernel_databases]
        _mpi_dirs = ['run%04d'%(task+1) for task in range(self.nproc)]

        # 2022-11-14
        # mkdirs runxxxx for mpi using.
        for dir_ in _required_structure:
            unix.mkdir(os.path.join(self.path.mainsolver, dir_))
        ...
        # Copy all input DATA/ files except the source and station files
        src = glob(os.path.join(self.path.specfem_data, "*"))
        src = [_ for _ in src if self.source_prefix not in _]
        src = [_ for _ in src if self.station_prefix not in _]
        dst = os.path.join(self.path.mainsolver, "DATA", "")
        unix.cp(src, dst)

        # mkdirs for iters for solving the IO problem
        iters = ceil(self.ntask/(self.nproc)) # The number of iters of all GPU.
        for _iter in range(iters):
            src = self.path.mainsolver
            dst = os.path.join(self.path.solver, "mainsolver_iter%04d"%(_iter))
            if os.path.exists(dst):
                os.system(f"rm -r {dst}")
            os.system(f"cp -r {src} {dst}")

            # WRITE MPI SERVICES
            service_path = os.path.join(dst, "SERVICES")
            if not os.path.exists(service_path):
                with open(service_path, "w") as f:
                    task_per_node_per_gpu = int (self.nproc//2)
                    #f.write(f"gitls164:{self.nproc}\n")
                    #f.write("gitls165:0\n")
                    f.write(f"gitls164:{task_per_node_per_gpu}\n")
                    f.write(f"gitls165:{task_per_node_per_gpu}\n")

        # Link all the mainsolver_iterxxxx direcotorys to the xxxx folders
        source_names = ["%04d"%(idx+1) for idx in range(self.ntask)]
        # runing dirs the runxxxx folders
        _run_dirs = ['run%04d'%(task+1) for task in range(self.nproc)]
        for _iter in range(iters):
            # Get the path of mainsolver
            mainsolver_path = os.path.join (self.path.solver, "mainsolver_iter%04d"%(_iter))
            if (_iter+1)*self.nproc < len(source_names):
                source_names_need_resolve = source_names[_iter*self.nproc:(_iter+1)*self.nproc]
            else:
                source_names_need_resolve = source_names[_iter*self.nproc::]
            for dir_, todo_source_name in zip(_run_dirs, source_names_need_resolve):
                src = os.path.join(self.path.scratch, todo_source_name)
                dst = os.path.join(mainsolver_path, dir_)
                if os.path.exists(dst):
                    unix.rm(dst)
                os.symlink(src, dst)
    ...
    def _initialize_working_directories(self):
        ...
        _required_structure = {"DATA", "traces/obs", "traces/syn",
                               "traces/adj", self.model_databases,
                               self.kernel_databases}

        # Initialize blank folders
        for idx, source_name in enumerate(self.source_names):
            ...
            if idx > 0:
                unix.cp(src = cwd_template, dst = cwd)
    ...
    def _initialize_working_directory(self, cwd=None):
        ...
        if int (source_name) == 1:
            logger.debug(f"initializing solver directories.")
        ...
        # Symlink event station specifically, only retain station prefix
        src = os.path.join(self.path.specfem_data,
                           f"{self.station_prefix}_{source_name}")
        dst = os.path.join(cwd, "DATA", self.station_prefix)
        if not os.path.exists(dst):
            unix.ln(src, dst)
