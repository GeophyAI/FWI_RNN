...
def NormalizedCrossCorrelation(syn, obs, nt, dt, line_search=True):

    if (np.max(np.abs(obs))) >0:
        l2normsyn = np.linalg.norm(syn, ord=2)
        l2normobs = np.linalg.norm(obs, ord=2)
        nsyn = syn/l2normsyn
        nobs = obs/l2normobs
        if not line_search:
            return (np.dot(nsyn, nobs)*nsyn-nobs)/l2normsyn
        else:
            return (np.dot(nsyn, nobs)*nsyn+nobs)/l2normsyn
    else:
        return obs
