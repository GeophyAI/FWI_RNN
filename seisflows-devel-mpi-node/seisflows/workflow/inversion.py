    ...
    ***
    ...
    def run(self):
        ...
        while self.iteration < self.end + 1:
            ...
            logger.info(msg.mnr("VERSION: 2022-12-03 MORNING"))
    ...
    def evaluate_objective_function_for_line_search(self, save_residuals=False, **kwargs):
        """
        Overwrite evaluate objective function to include MORE input parameters
        specifying which evaluation in the inversion we are at. Also removes
        the check for a preprocessing module because it is assumed we have a
        ...
        self.preprocess.quantify_misfit_line_search(
            source_name=self.solver.source_name,
            source_names=self.solver.source_names,
            save_residuals=save_residuals,
            iteration=self.iteration,
    ...
    def initialize_line_search(self):
        ...
        logger.info("Saving m_try to eval_func")
    ...
    def perform_line_search(self):
        ...
        if status.upper() == "PASS":
            ...
            self.optimize.save_vector("m_try", m_try)
            # Expose model `m_try` to the solver by placing it in eval_func dir.
            logger.info("Replacing m_try files, save into eval_func")
            m_try.write(path=os.path.join(self.path.eval_func, "model"))
            del m_try  # clear potentially large model vector from memory
    ...
    def _evaluate_line_search_misfit(self):
        ...
        if self.optimize.line_search_method.upper()=="CUSTOMER":
            logger.debug(f"evaluating line search misfit for calculating alpha")
            self.system.run(
                [self.run_forward_simulations,
                 self.evaluate_objective_function_for_line_search,],
            path_model=os.path.join(self.path.eval_func, "model"),
            save_residuals=os.path.join(self.path.eval_func, "residuals_line_search.txt"),
            no_backward=True,
        )
            total_alpha = np.loadtxt(os.path.join(self.path.eval_func, "residuals_line_search.txt"))
            alpha = abs(np.sum(total_alpha[:,0]) / np.sum(total_alpha[:,1]))
            self.optimize.save_vector(name="alpha_customer", m=alpha)
        else:
            self.system.run(
                [self.run_forward_simulations,
                 self.evaluate_objective_function],
