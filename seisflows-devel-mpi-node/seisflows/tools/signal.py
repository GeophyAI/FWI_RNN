...
def mask_customer(slope, const, offset, nt, dt, vel_speed=1600.):
    """
    ...
    # Caculate time offsets
    itmin = 2.
    itmax = int(abs(offset)/vel_speed/dt+0.3/dt)
...
def mute_arrivals(st, slope, const, choice):
    ...
    s_coords = np.array(get_source_coords(st))
    r_coords = np.array(get_receiver_coords(st))
    ...
    for i, tr in enumerate(st_out):
        ...
        mask_arr = mask_customer(slope=slope, const=const, offset=offset, nt=nt, dt=dt)
        if choice.lower() == "early":
            tr.data *= mask_arr
        elif choice.lower() == "late":
            tr.data *= 1 - mask_arr
