    ...
    if args.mode == 'inversion':
        ...
        for epoch in tqdm.tqdm(range(0, cfg['training']['N_epochs'])):
            ...
            if rank==0:
                ...
                np.save('/disk7/sharingCenter/DATA/FWI/Marmousi_water2/Inversion_bytorch/Test01/grad_%02depoch.npy'%(epoch), grad_root)
                loss = optimizer.step()
                torch.save(model.state_dict()['cell.geom.vel'], '/disk7/sharingCenter/DATA/FWI/Marmousi_water2/Inversion_bytorch/Test01/%02depoch.pth'%(epoch))
